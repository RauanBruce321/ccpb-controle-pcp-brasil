...
async function init(){loginFlows();state=await loadState();if(!state.users?.length)state.users=DEFAULT_USERS;state.users.forEach(u=>{u.permissions=Array.isArray(u.permissions)?u.permissions:[];if(u.role==='admin')u.permissions=[...PERMISSIONS];if(u.role==='triage'&&!u.permissions.includes('triage.multipleNotesPosition'))u.permissions.push('triage.multipleNotesPosition')});if(!state.records)state.records=[];if(!state.positions)state.positions=[];if(!state.logs)state.logs=[];if(!state.shiftRestarts)state.shiftRestarts=[];if(!state.config)state.config=emptyState().config;rebuildCache();$('#loginForm').onsubmit=login;$('#logoutBtn').onclick=logout;$('#menuBtn').onclick=()=>$('#sidebar').classList.toggle('open');$('#tvBtn').onclick=toggleTV;$('#globalSearchBtn').onclick=globalSearch;startInactivityWatch();$('#towerFile').onchange=e=>{const f=e.target.files[0];e.target.value='';importTower(f)};addEventListener('online',onlineUpdate);addEventListener('offline',onlineUpdate);onlineUpdate();setInterval(()=>{$('#shiftBadge').textContent=shiftOf();firebasePull().catch(e=>console.error('Falha ao atualizar Firebase:',e))},3000);sessionStorage.getItem('ccpb_session')||'null');if(saved){const user=state.users.find(u=>u.username===saved.username&&u.active);if(user){session={user,operator:saved.operator||user.name};enterApp()}}setTimeout(()=>{$('#splash').classList.add('hidden');if(!session)$('#loginView').classList.remove('hidden')},700)}
